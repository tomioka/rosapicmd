...
let validate_endpoint e =
  if (List.mem endpoints e ~equal:String.equal) then e
    ...
    raise (Invalid_argument
             (sprintf "Unknown endpoint %s; expected: %s" e (String.concat ~sep:", " endpoints)))

let connect endpoint language local_connection adm_output input devel verbose () =
  let uri = if local_connection
    then
      "http://localhost:8181"
    else
      "https://api.rosette.com"
  in
  let uri = sprintf "%s/rest/v1/%s" uri endpoint
            |> Uri.of_string in
  let uri = if adm_output
    then Uri.with_query uri [("output", ["rosette"])]
    else uri
  in
  let headers = [] in
  let headers = if devel then headers @ [("X-RosetteAPI-Devel", "true")] else headers in
  let headers = match Sys.getenv "ROSAPI_KEY" with
